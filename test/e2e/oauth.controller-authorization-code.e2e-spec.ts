...
describe.skip("oauth2 authorization_code e2e", () => {
  ...
  it.skip("allows auth code grant with PKCE S256", async () => {
    ...
    const authorizeResponse = await request(http).get("/oauth2/authorize").query({
      response_type: "code",
      client_id: client.id,
    ...
    const decodedCode: IAuthCodePayload | any = jwtDecode(authorizeResponseQuery.code);
    ...
    const tokenResponse = await request(http).post("/oauth2/token").send({
