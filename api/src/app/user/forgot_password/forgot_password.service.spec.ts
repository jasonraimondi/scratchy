...
describe(ForgotPasswordService.name, () => {
  ...
  it("#updatePasswordFromToken success", async () => {
    ...
    // assert
    const updatedUser = User.fromPrisma(await testingModule.prisma.user.findUniqueOrThrow({ where: { id: user.id } }));
    await expect(updatedUser.verify("my-new-password")).resolves.toBeUndefined();
    await expect(repository.findById(forgotPassword.id)).rejects.toThrowError(/No UserToken found/);
  });
});
